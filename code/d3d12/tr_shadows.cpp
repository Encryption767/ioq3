...
// VULKAN
// DX12
static void R_Dx_RenderShadowEdges(ID3D12PipelineState *dx_pipeline) {
...
void RB_ShadowTessEnd(void) {
	...
	if( backEnd.viewParms.isMirror ) {
		...
		// DX12
		R_Dx_RenderShadowEdges(dx.shadow_volume_pipelines[0][1]);
		R_Dx_RenderShadowEdges(dx.shadow_volume_pipelines[1][1]);
	} else {
		qglCullFace(GL_BACK);
		qglStencilOp(GL_KEEP, GL_KEEP, GL_INCR);
		R_GL_RenderShadowEdges();
		...
		// DX12
		R_Dx_RenderShadowEdges(dx.shadow_volume_pipelines[0][0]);
		R_Dx_RenderShadowEdges(dx.shadow_volume_pipelines[1][0]);
	}
