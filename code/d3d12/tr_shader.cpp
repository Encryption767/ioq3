...
static shader_t *FinishShader(void) {
	...
	// DX12: create pipelines for each shader stage
	...
	for( int i = 0; i < stage; i++ ) {
		...
		if( pStage->bundle[1].image[0] == nullptr )
			...
			def.shader_type = Dx_Shader_Type::multi_texture_mul;
		else if( shader.multitextureEnv == GL_ADD )
			def.shader_type = Dx_Shader_Type::multi_texture_add;
		else
...
shader_t *R_FindShaderByName(const char *name) {
	...
	COM_StripExtension(name, strippedName, 0);
